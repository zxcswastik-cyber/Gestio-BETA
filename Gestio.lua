...
-- Version: 5.4.4 Enterprise Stable Edition (Delta Sandbox Fixed)
...
local function getSafeGui()
    ...
    if s and res then return res end
    return (player and player:WaitForChild("PlayerGui", 2)) or CoreGui
end
...
table.insert(connections, RunService.Heartbeat:Connect(function(dt)
    ...
    if bunnyHopEnabled then
        ...
        elseif not grounded and bhopAirStrafe and hum.MoveDirection.Magnitude > 0.05 then
    ...
    if autoStrafeEnabled and not isPlayerGrounded(char, hrp) then
        ...
        if state == Enum.HumanoidStateType.Jumping or state == Enum.HumanoidStateType.Freefall then
            if hum.MoveDirection.Magnitude > 0 and camera then
                ...
                local fwd = camLook.Magnitude > 0 and camLook.Unit or Vector3.new(0,0,0)
                local rgt = camRight.Magnitude > 0 and camRight.Unit or Vector3.new(0,0,0)

                local fAmount = hum.MoveDirection:Dot(fwd)
                local rAmount = hum.MoveDirection:Dot(rgt)
...
addCard(eWorldSection, "Jump Circle", jumpCircleEnabled, function(v) 
    ...
    if v and player and player.Character then
        ...
        if activeJumpCircleData and activeJumpCircleData.Container then
