...
-- Crystal Outline Chams & Radiation
...
-- Grenade Trajectory & Radius Settings
...
-- RCS (Recoil Compensation System)
...
-- Mobile Bhop Engine Configuration
...
-- ==========================================
-- CRYSTAL ORBIT & RED RADIANCE ENGINE
-- ==========================================
...
-- ==========================================
-- TARGET SELECTION & RAYCAST CACHING
-- ==========================================
...
-- ==========================================
-- 2D SCREEN OVERLAY MODULE (ENEMIES ONLY)
-- ==========================================
...
table.insert(connections, RunService.RenderStepped:Connect(function(dt)
            ...
            if chams then
                ...
                if chamsEnabled and rootPart then
                    ...
                    for idx, shard in ipairs(chams.Crystals) do
                        local angleOffset = (idx / count) * (math.pi * 2)
                        local currentAngle = timeTick + angleOffset
                        
                        local x = math.cos(currentAngle) * crystalOrbitRadius
                        local z = math.sin(currentAngle) * crystalOrbitRadius
                        local y = math.sin(timeTick * 1.5 + idx) * 0.75
                        
                        local targetCFrame = CFrame.new(rootPos + Vector3.new(x, y, z)) * CFrame.Angles(timeTick, currentAngle, 0)
                        shard.CFrame = targetCFrame
                        ...
                        if light then
                            ...
                            light.Color = currentTheme.CrystalLightColor
                        end
                    end
                else
                    for idx, shard in ipairs(chams.Crystals) do
                        shard.Transparency = 1.0
                        if chams.Lights[idx] then
                            chams.Lights[idx].Enabled = false
                        end
                    end
                end
            end
    ...
    -- Classic Environment Controllers
