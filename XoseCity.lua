...
-- PANDA AUTH / PUSL-V4
-- ==========================================
local function runXCPandaAuth()
    local loaded, PUSL = pcall(function()
        local source = game:HttpGet("https://secure.pandauth.com/pv4/lib")
        local loader = loadstring(source)
        assert(type(loader) == "function", "invalid Panda library")
        return loader()
    end)
    if not loaded or type(PUSL) ~= "table" or type(PUSL.configure) ~= "function"
        or type(PUSL.validate) ~= "function" then
        warn("[XC/Panda] Library failed to initialize: " .. tostring(PUSL))
        return false
    end

    local configured, configureError = pcall(PUSL.configure, {serviceId = "xosecity"})
    if not configured then
        warn("[XC/Panda] Configuration failed: " .. tostring(configureError))
        return false
    end

    local authEnv = type(getgenv) == "function" and getgenv() or _G
    local function validateKey(value)
        value = tostring(value or ""):match("^%s*(.-)%s*$")
        if value == "" then return false, "Enter your key" end
        local ok, result = pcall(PUSL.validate, value)
        if not ok then return false, tostring(result) end
        if type(result) ~= "table" or result.success ~= true then
            return false, type(result) == "table" and tostring(result.message or result.error or "Invalid key") or "Invalid key"
        end
        authEnv.XC_PANDA_KEY = value
        return true, result
    end

    if authEnv.XC_PANDA_KEY then
        local valid = validateKey(authEnv.XC_PANDA_KEY)
        if valid then return true end
        authEnv.XC_PANDA_KEY = nil
    end
    ...
    if type(gethui) == "function" then
        ...
        if ok and hiddenUi then parent = hiddenUi end
    elseif localPlayer then
        parent = localPlayer:WaitForChild("PlayerGui")
    end

    pcall(function()
        local previous = parent:FindFirstChild("XCPandaAuth")
    ...
    elseif not parented then
        warn("[XC/Panda] Unable to create authentication UI")
        gui:Destroy()
        return false
    end

    local card = Instance.new("Frame")
    card.Size = UDim2.fromOffset(330, 205)
    card.Position = UDim2.new(0.5, -165, 0.5, -102)
    card.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
    card.BorderColor3 = Color3.fromRGB(152, 204, 0)
    ...
    accent.Position = UDim2.fromOffset(12, 42)
    ...
    validateButton.Size = UDim2.new(0.55, -16, 0, 34)
    validateButton.Position = UDim2.fromOffset(12, 108)
    validateButton.BackgroundColor3 = Color3.fromRGB(152, 204, 0)
    validateButton.BorderSizePixel = 0
    validateButton.Text = "VALIDATE"
    validateButton.TextColor3 = Color3.fromRGB(8, 8, 8)
    validateButton.Font = Enum.Font.GothamBold
    validateButton.TextSize = 12
    validateButton.Parent = card

    local getKeyButton = Instance.new("TextButton")
    getKeyButton.Size = UDim2.new(0.45, -8, 0, 34)
    getKeyButton.Position = UDim2.new(0.55, 4, 0, 108)
    getKeyButton.BackgroundColor3 = Color3.fromRGB(28, 28, 28)
    getKeyButton.BorderColor3 = Color3.fromRGB(60, 60, 60)
    getKeyButton.Text = "GET KEY"
    getKeyButton.TextColor3 = Color3.fromRGB(235, 235, 235)
    ...
    status.Text = "Enter a valid key to start XC"
    ...
    validateButton.Activated:Connect(function()
        ...
        task.spawn(function()
            local valid, result = validateKey(keyBox.Text)
            if valid then
                status.TextColor3 = Color3.fromRGB(152, 204, 0)
                status.Text = result.isPremium and "Authenticated • Premium" or "Authenticated"
                task.wait(0.2)
                completed:Fire(true)
                return
            end
            status.TextColor3 = Color3.fromRGB(220, 82, 82)
    ...
    getKeyButton.Activated:Connect(function()
        ...
        if not ok or type(url) ~= "string" then
            status.TextColor3 = Color3.fromRGB(220, 82, 82)
            status.Text = "Unable to create key link"
            return
        end
    ...
    local authenticated = completed.Event:Wait()
    completed:Destroy()
    gui:Destroy()
    return authenticated == true
end

if not runXCPandaAuth() then return end
